...
class inventory_stock_rental_detail_model extends CI_Model{   
	...
	function update_single($rental_stock_header_id, $rental_stock_id, $data)
	{
	...
	function fetch_all_active_by_rental_stock_header_id($rental_stock_header_id)
	{
		...
		$query = $this->db->query("SELECT inventory_stock_rental_detail.rental_stock_id, inventory_stock_rental_detail.rental_stock_header_id, inventory_stock_rental_detail.item_id, inventory_stock_rental_detail.full_stock_count,  inventory_stock_rental_detail.is_sub_item, IF(inventory_stock_rental_detail.is_sub_item = 0, inventory_item.item_name, inventory_sub_item.sub_item_name)  as item_name FROM `inventory_stock_rental_detail` left join inventory_item on inventory_stock_rental_detail.item_id = inventory_item.item_id left join inventory_sub_item on inventory_stock_rental_detail.item_id = inventory_sub_item.sub_item_id WHERE `rental_stock_header_id` = '$rental_stock_header_id' AND inventory_stock_rental_detail.is_active_rental_stock_detail = 1;");
