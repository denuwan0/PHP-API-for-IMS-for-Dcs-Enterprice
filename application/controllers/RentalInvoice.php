...
class RentalInvoice extends CI_Controller {
	...
	function printInvoice(){
		...
		if($this->input->get('id'))
		{
			...
			$deposite_amount = $invoiceData[0]["deposite_amount"];
						
			$customerData  = $this->Customer_model->fetch_single($invoiceData[0]['customer_id']);
			
			$itemData  = $this->Inventory_rental_invoice_detail_model->fetch_all_by_invoice_id($invoice_no);
			...
			$message = file_get_contents(base_url().'assets/template/rentalInvoice.html'); 
			...
			if($phparray["stockHeader"][0]->is_active_inv_stock_retail == 1 && $phparray["stockHeader"][0]->is_approved_inv_stock_retail == 1){
				...
				foreach($itemArray as $value){
					...
					if(empty($totStockData)){
						...
						$this->Inventory_rental_total_stock_model->update_single($retail_stock_id, $itemData1);
					}
	...
	{	
		...
		$data = $this->Inventory_rental_total_stock_model->get_retail_item_details_by_item_id_branch_id_is_sub_item($item_id, $emp_branch_id, $is_sub_item);
	...
	function fetch_single_join_by_invoice_id()
	{
		...
		{			
			$retail_stock_header_id = $this->input->get('id');
			$data1 = $this->Inventory_rental_invoice_header_model->fetch_invoice_header_by_invoice_id($retail_stock_header_id)->result_array();
			
			//var_dump($data1);
			
			$retail_stock_header_id = $this->input->get('id');
			$data2 = $this->Inventory_rental_invoice_header_model->fetch_invoice_detail_by_invoice_id($retail_stock_header_id)->result_array();
