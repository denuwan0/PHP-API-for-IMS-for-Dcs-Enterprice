...
class VehicleService extends CI_Controller {
	...
	function fetch_single_all_join()
	{
		...
		{			
			$id = $this->input->get('id');
			$data = $this->Vehicle_service_details_model->fetch_single_all_join($id);
			//var_dump($data);
			echo json_encode($data->result_array());
		}
	}
		...
		{			
			if($this->input->post('is_active_vhcl_srv_detail') == 0){	
				...
				if($status>0){
					...
					$this->Vehicle_service_details_model->update_single($this->input->post('service_detail_id'), $data);
