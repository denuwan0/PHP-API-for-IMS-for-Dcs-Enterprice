...
class EmpMedicalRecord extends CI_Controller {
	...
	function index()
	{
		$data = $this->Emp_medical_records_model->fetch_all();
	...
	{		
		...
		if($this->form_validation->run())
		{
			...
			$this->Emp_medical_records_model->insert($data);
	...
	{		
		$data = $this->Emp_medical_records_model->fetch_all_active();
		echo json_encode($data->result_array());
	...
	function fetch_single()
	{
		...
		{			
			...
			$data = $this->Emp_medical_records_model->fetch_single($id);
	...
	function fetch_single_join()
	{
		...
		{			
			$id = $this->input->get('id');
			$data = $this->Emp_medical_records_model->fetch_single_join($id);
	...
	{	
		...
		if($user_group_name == "Admin"){			
			$data = $this->Emp_medical_records_model->fetch_all_join();
		}
		else if($user_group_name == "Manager"){
			$data = $this->Emp_medical_records_model->fetch_all_join_by_branch_id($branch_id);
		}
		else if($user_group_name == "Staff"){		
			$data = $this->Emp_medical_records_model->fetch_single_join_by_emp_id($emp_id);
		}
		...
		{			
			if($this->input->post('is_active_medical_records') == 0){	
				...
				if($status>0){
					...
					$this->Emp_medical_records_model->update_single($this->input->post('med_record_id'), $data);
	...
	function delete()
	{
		...
		{
			if($this->Emp_medical_records_model->delete_single($this->input->post('id')))
			{
				$array = array(
